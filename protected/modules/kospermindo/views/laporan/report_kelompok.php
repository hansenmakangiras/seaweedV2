	...
	<?php $i = 0; $total_panen = 0; foreach ($jenis_komoditi as $key => $valjnsk) { 
		...
		<div class="col-sm-3">
			...
			<div class="tile-stats tile-<?= $colors[$i] ?>">
				...
				<div id="<?= $valjnsk['id'] ?>" class="num" data-start="0" data-end="<?= !empty($allFarmers) ? $allFarmers : "0";?>" data-postfix="" data-duration="1500"
						 data-delay="1200" data-color-hex="<?= $colorHex[$i] ?>"><?= (!is_null($valjnsk['jumlah'][0]['SUM(total_panen)'])) ? number_format((float)$valjnsk['jumlah'][0]['SUM(total_panen)']/1000, 2, '.', '') :'0' ?>
				</div>
				</b> TON</h2>
				...
				<tr class="count-footer">
					<th width="50%" class="col-padding-1">
						<div class="pull-left">
							...
							<small style="font-size: 20px; font-weight: bold; color: #00a65a" id="total_panen"><?= $total_panen; ?> Ton</small>
						</div>
					</th>
					<th width="50%" class="col-padding-1">
						<div class="pull-left">
...
<?php
	Yii::app()->clientScript->registerScript('graph', '
		...
		$(document).ready(function(){
			...
			$.ajax({
				type: "POST",
				url: "/kospermindo/laporan/chartkelompok",
				data:{
			...
			$("#gudang").on("change", function(){
				$.ajax({
					...
					success: function(data){
						...
						$.ajax({
							type: "POST",
							url: "/kospermindo/laporan/getkelompok",
							data:{
								"id_gudang" : $("#gudang").val(),
								"id_kelompok" : $("#kelompok").val(),
								"start" : $("input[name=daterangepicker_start]").val(),
