...
var kospermindoMessages = kospermindoMessages || {
        $current_user: null,
        isOpen: false,
        chat_history: [],
        statuses: {
...
(function ($, window, undefined) {
    ...
    $.extend(kospermindoMessages, {

        init: function () {
            // Conversation Close
            $conversation_window.on('click', '.conversation-close', kospermindoMessages.close);
            ...
            $("body").on('click', '.chat-close', function (ev) {
                ...
                kospermindoMessages.hideChat(animate);
            });
            ...
            $("body").on('click', '.chat-open', function (ev) {
                ...
                kospermindoMessages.showChat(animate);
            });
            ...
            $textarea.keydown(function (e) {
                if (e.keyCode == 13 && !e.shiftKey) {
                    e.preventDefault();
                    kospermindoMessages.submitMessage();
                    return false;
                }
                else if (e.keyCode == 27) {
                    kospermindoMessages.close();
                }
            });
            ...
            $chat.on('click', '.chat-group a', function (ev) {
                ...
                if ($chat_user.hasClass('active')) {
                    ...
                    kospermindoMessages.open($chat_user);
                }
            });
            ...
            kospermindoMessages.refreshUserIds();
            kospermindoMessages.orderGroups();
            kospermindoMessages.prefetchMessages();
            kospermindoMessages.countUnreadMessages();
            kospermindoMessages.puffUnreads();
            ...
            $("body").on('click', '[data-toggle="chat"]', function (ev) {
                ...
                kospermindoMessages.toggleChat(with_animation, collapse_sidebar);
            });
        },
        ...
        refreshUserIds: function () {
            $('#chat .chat-group a').each(function (i, el) {
                ...
                if (typeof kospermindoMessages.chat_history[id] == 'undefined') {
                    var status = $this.data('status');

                    if (!status) {
                        for (var i in kospermindoMessages.statuses) {
                            if ($status.hasClass(kospermindoMessages.statuses[i].class)) {
                                status = i;
                                break;
                            }
                        }
                    ...
                    kospermindoMessages.chat_history[id] = {
                        $el: $this,
                        messages: [],
                        unreads: 0,
                        status: status
        ...
        getStatus: function (id) {
            ...
            var user = kospermindoMessages.chat_history[id];
        ...
        setStatus: function (id, status) {
            ...
            if ($user.length && this.statuses[status].class) {
                ...
                kospermindoMessages.chat_history[$user.attr('id')].status = status;
                this.orderGroups();
            }
        },
        ...
        orderGroups: function () {
            ...
            $groups.each(function (i, el) {
                ...
                $contacts.each(function (i, el) {
                    ...
                    for (var i in kospermindoMessages.statuses) {
                        var status = kospermindoMessages.statuses[i];
            ...
            $chat.find('.chat-group a').each(function (i, el) {
                ...
                if (history_container && history_container.length) {
                    $($(history_container)).find('> li').each(function (j, el2) {
                        ...
                        kospermindoMessages.pushMessage(id, message, from, time, fromOpponent, unread)
                    });
                }
        ...
        countUnreadMessages: function (id) {
            ...
            if (!id) {
                for (var id in kospermindoMessages.chat_history) {
                    var user = kospermindoMessages.chat_history[id],
        ...
        puffUnreads: function () {
            for (var i in this.chat_history) {
                var entry = kospermindoMessages.chat_history[i],
        ...
        toggleChat: function (animated, collapse_sidebar) {
            ...
            kospermindoMessages[_func](animated);
